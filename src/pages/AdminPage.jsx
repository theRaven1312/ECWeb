    ...
    <div className='flex flex-col px-8 gap-8'>
      ...
      <div className='flex gap-8'>
        <ul className='optionBar flex flex-col w-1/4 border-1 border-gray-300 p-4 gap-4 rounded-lg h-screen'>
          <li 
            className={`optionBar__item cursor-pointer p-2 rounded ${option === 'users' ? 'bg-gray-200 ' : 'hover:bg-gray-100'}`}
            onClick={() => setOption('users')}
          >
            Users
          </li>
          <li 
            className={`optionBar__item cursor-pointer p-2 rounded ${option === 'products' ? 'bg-gray-200' : 'hover:bg-gray-100'}`}
            onClick={() => setOption('products')}
          >
            Products
          </li>
          <li 
            className={`optionBar__item cursor-pointer p-2 rounded ${option === 'orders' ? 'bg-gray-200' : 'hover:bg-gray-100'}`}
            onClick={() => setOption('orders')}
          >
            Orders
          </li>
          <li 
            className={`optionBar__item cursor-pointer p-2 rounded ${option === 'sales' ? 'bg-gray-200' : 'hover:bg-gray-100'}`}
            onClick={() => setOption('sales')}
          >
            Sales
          </li>
          <li 
            className={`optionBar__item cursor-pointer p-2 rounded ${option === 'categories' ? 'bg-gray-200' : 'hover:bg-gray-100'}`}
